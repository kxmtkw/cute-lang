...
from Compiler.imagen.image import ImageBuilder, Format


class InstrSet(Enum):
	"""
	Instruction Set! 
	Instruction values store (opcode, [format1, format2, ...])
	where opcode is the opcode duh
	and the second element is a list with the number of arguments with their formats
	"""
	null     = (0x00, [])
	halt     = (0x01, [Format.u8])
...
class ReferenceUnit(ABC):
	"A unit that can be a reference of another unit."
	def __init__(self) -> None:
		pass

	@abstractmethod
	def refer(self, builder: ImageBuilder) -> Format:
		pass
...
class Instruction(CompileableUnit):
	...
	def __post_init__(self) -> None:
		expected_argument_count = len(self.details.value[1])
		if len(self.arguments) != expected_argument_count:
			raise ValueError(
				f"Instruction {self.details.name} requires {expected_argument_count} arguments; "
	...
	def __repr__(self) -> str:
		...
		return f"{self.details.name} {arguments}".rstrip()


	def assemble(self, builder: ImageBuilder):
		instruction, arg_fmts = self.details.value
...
class Label(CompileableUnit, ReferenceUnit):
	"A label. Labels with the same id are considered the same label."
	id: int
...
class Constant(CompileableUnit, ReferenceUnit):
	"A constant to be added to the data blob."
